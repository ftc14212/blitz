...
public class turretTracking extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        AnalogInput elc = hardwareMap.get(AnalogInput.class, "elc");
        // servos
        ...
        while (opModeIsActive()) {
            ...
            // turret control
            turretTpos = turretOn ? wrap(alignTurret(
                    follower.getPose().getX(),
                    follower.getPose().getY(),
                    Math.toDegrees(follower.getHeading()),
                    target)) : 0;
            // turret code
            double encoderDeg = (((elc.getVoltage() / PIDTuneTurretAnalog.MAX_VOLTAGE) * 360.0 + PIDTuneTurretAnalog.OFFSET_DEG) % 360 + 360) % 360;
            double turretCpos = (encoderDeg + turnCount * 360.0) * PIDTuneTurretAnalog.GEAR_RATIO;
            if (firstLoop) {
                lastEncoderDeg = encoderDeg;
                firstLoop = false;
            }
            double delta = encoderDeg - lastEncoderDeg;
            if (delta > 180) turnCount--;
            else if (delta < -180) turnCount++;
            lastEncoderDeg = encoderDeg;
            ...
            telemetryM.addData(true, "turnCount", turnCount);
            telemetryM.addData(true, "voltage", elc.getVoltage());
